...
const Dashboard2 = () => {
    ...
    const [activeButton, setActiveButton] = useState('free'); // Track active button state
    ...
    const freeButton = (id, update) => {
        ...
        fetch(`http://localhost:5000/users`, {
            ...
            .then(data => {
                refetch();
                setFreeBusy(update);
                setActiveButton(update); // Update active button state
            });
    };
        ...
        <>
            <div className="d-flex  mt-2">
                <div
                    ...
                    className={`p-2 flex-fill text-center text-white ${activeButton === 'free' ? 'bg-primary' : 'bg-success'}`}
                >
                    Free
                </div>
                <div
                    ...
                    className={`p-2 flex-fill text-center text-white ${activeButton === 'busy' ? 'bg-primary' : 'bg-danger'}`}
                >
                    Busy
                </div>
            </div>
            ...
            {user
                .filter(item => item.timeManagment === freeBusy)
                .map(item => (
                    <div
                        key={item._id}
                        className="mt-2 d-flex justify-content-between align-items-center p-2 gap-2"
